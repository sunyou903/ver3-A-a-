  ...
  // ===== 실행 =====
   ...
   async function run() {
     try {
       ...
       // 요약만 로그
       if (A?.summary) console.log('[A]', A.summary);
       if (B?.summary) console.log('[B]', B.summary);
       if (C?.summary) console.log('[C]', C.summary);
   
       const base = f.name.replace(/\.[^.]+$/, '');
       // saveAsOneWorkbook은 A만 필수, B/C는 null 가능
